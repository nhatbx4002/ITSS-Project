...
export default function UpcomingWorkoutsPage() {
  ...
  useEffect(() => {
    const fetchWorkouts = async () => {
      try {
        const response = await memberApi.getUpcomingWorkouts("6811f30ee8de8e72ec3291c4"); // Thay USER_ID bằng ID thực tế
        if (response.success) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Upcoming Workouts</h1>
      </div>
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCancelWorkout} variant="destructive">Cancel Workout</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
              ...
              <tbody>
                {filteredWorkouts.map((workout, index) => (
                  <tr key={workout._id} className="border-b">
                    ...
                    <td className="py-3 px-4">Gym Floor</td>
